...
- 分身 EVA.md 应**轻量但完整**——任务背景和相关技能/知识能让分身更快进入状态
- 不要 cp 主 EVA 的 EVA.md，里面可能索引了大量本地文件，分身不需要
- API key、BASE_URL 等通过环境变量 `EVA_API_KEY`、`EVA_BASE_URL` 自动继承，不需要写入 EVA.md
- **始终在 spawn 前写好 EVA.md**，不要让分身带着空 EVA.md 启动

### 分身可以再生分身（递归）
分身也是完整的 EVA，只要分身自己的 EVA.md 里写了 spawn 技巧，分身内部同样可以再生子分身，形成分身树。
