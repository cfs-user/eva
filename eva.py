...
- 在最后一步leave_memory_hints被执行后，消息历史将被压缩，其中最后一条用户消息之后的对话片段将被保留、工具结果会被截断、reasoning_content会被清空。你将【仅基于你自己留下的记忆线索】继续开展任务
...
leave_memory_hints遵循如下处理流程：（1）在消息历史中定位到“紧急危机”提示，将危机提示到最近一条用户消息之前所有对话片段保留下来，但会对其中的工具返回结果做 200 字符截断、对assistant的推理链（reasoning_content）做删除；（2）然后把整个消息列表重置为“系统提示 + 压缩后的片段 + 一条记忆已耗尽的提示”；（3）最后把用户指定的记忆线索写入 hint 文件。""",
            "parameters": {
                "type": "object",
                "properties": {
                    "hints": {"type": "string"},
